...
public class PileLayout2 extends ViewGroup {
    ...
    public PileLayout2(Context context) {
        this(context, null);
    }

    public PileLayout2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PileLayout2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        everyWidth = width-interval*(VIEW_COUNT-1);
        switch (View.MeasureSpec.getMode(heightMeasureSpec)){
            case MeasureSpec.AT_MOST:
                case MeasureSpec.UNSPECIFIED:
                    everyHeight= (int)(everyWidth*1.3f);
                break;
            case MeasureSpec.EXACTLY:
                everyHeight=View.resolveSizeAndState(View.MeasureSpec.getSize(heightMeasureSpec), heightMeasureSpec, 0);
                break;
        }
        setMeasuredDimension(width,everyHeight+getPaddingBottom()+getPaddingBottom());
        ...
        if (originX.size() == 0) {
            for(int i=0;i<VIEW_COUNT;i++){
                if(i==VIEW_COUNT-1){
                    originX.add(-everyWidth-interval);
                }if(i==VIEW_COUNT-2){
                    originX.add(-everyWidth);
                }else {
                    originX.add(interval*(VIEW_COUNT-i-1));
                }
            }
            scrollDistanceMax = Math.abs(originX.get(VIEW_COUNT-2)-originX.get(VIEW_COUNT-3));
        }
    }
    ...
    private void adjustAlpha(View itemView) {
        ...
        if (itemView.getLeft() > position3) {
            float alpha=1-(itemView.getLeft()-position3) /Float.valueOf(originX.get(0)-interval);
            if(alpha>0.9){
                alpha=1f;
            }
            itemView.setAlpha(alpha);

        } else if(itemView.getLeft() < position3){
            int position4 = originX.get(4);
            float alpha =1f-Math.abs(Float.valueOf(itemView.getLeft())/ (position4 - position3)) ;
            itemView.setAlpha(alpha);
        }else {
            itemView.setAlpha(1);
        }
    ...
    private void adjustScale(View itemView) {
        ...
        if (thisLeft < position3) {
                scale=1f-((position3-itemView.getLeft())/Float.valueOf(scrollDistanceMax*2));
        }else if(thisLeft > position3){
            scale=1f-(itemView.getLeft()-position3)*scaleStep/interval;
        }
    ...
    private void onRelease(float eventX, int velocityX) {
        ...
        if (velocityX < -VELOCITY_THRESHOLD || (animatingView.getLeft() < originX.get(3) - scrollDistanceMax /2)) {
            destX = originX.get(4);
            tag++;
    ...
    private  void requireScrollChange(int dx) {
        ...
        if (dx < 0 ) {
            ...
            if (currentPosition <0) {
              ...
              if (currentPosition == 0) {
                if (getChildAt(3).getLeft() + dx >= originX.get(3)) {
                    dx = originX.get(3) - getChildAt(3).getLeft();
                }
            }
        }
        ...
        float rate = (float) ((view1.getLeft() + dx) - originX.get(4)) / scrollDistanceMax;
        if (rate < 0) {
            rate = 0;
        }
        if (rate > 1) {
        ...
        int position1 = Math.round((1-rate )* (originX.get(1) - originX.get(2))) + originX.get(2);
        boolean endAnim = false;
        if (position1 >= originX.get(1) && null != animatingView) {
            //动画结束纠正布局
            animator.cancel();
            endAnim = true;
        }
        for (int i = 0; i < num; i++) {
            ...
            if (endAnim) {
                ...
                itemView.offsetLeftAndRight(dx);
                 ...
                 int   position =Math.round((1-rate)* (originX.get(i+1) - originX.get(i)))+ originX.get(i);
    ...
    private void doBindAdapter() {
        ...
        if (getChildCount() == 0) {
            ...
            for (int i = 0; i < VIEW_COUNT-1; i++) {
                ...
                addView(frameLayout);
                frameLayout.setTag(3-i); // 这个tag主要是对应于在dataList中的数据index
        ...
        for (int i = 0; i < num-1; i++) {
            ...
            if (3-i< adapter.getItemCount()) {
                frameLayout.setVisibility(View.VISIBLE);
                adapter.bindView(frameLayout.getChildAt(0), 3-i);
            } else {
